...
<body>
  ...
  <div class="flex items-center justify-center min-h-screen flex-col gap-4">
    ...
    <form class="border rounded-lg p-6 flex flex-col gap-2 shadow-lg" action="/signup" method="post">
      @csrf 
      <label class="font-semibold text-sm" for="name">Name</label>
      <input class="border px-4 py-2 rounded-md" autofocus autocomplete="off" type="text" name="name" id="name" >
    ...
    <div class="flex items-center justify-center gap-2">
      <p class="text-xs font-medium">Have an account? </p>
      <a class="text-xs font-medium underline" href="/login">Sign in</a>
    </div>
    @if ($errors->any())
